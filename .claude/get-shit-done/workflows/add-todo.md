...
```bash
INIT=$(node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" init todos)
...
```bash
slug=$(node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
