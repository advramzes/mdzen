...
```bash
INIT=$(node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-doc-writer 2>/dev/null)
```
...
```bash
node "F:/MDZen/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs: generate project documentation" \
