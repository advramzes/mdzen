...
- **Global scope**: `F:/MDZen/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/MDZen/.claude/notes/`)
...
2. Glob `F:/MDZen/.claude/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `F:/MDZen/.claude/notes/` — works in any directory
